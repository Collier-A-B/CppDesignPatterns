...
{
    private:
        std::unique_ptr<VirtualDrive> adaptee;
        int generateUID() 
        {
    ...
    public:
        VirtualDriveAdapter() {
            adaptee = make_unique<VirtualDrive>();
        }
        bool uploadContents(const string& content) override
        {
            return adaptee->uploadData(content, generateUID());
        }
        int getFreeSpace() override
        {
            int size = adaptee->totalSpace - adaptee->usedSpace();
